...
class BotGoal:
    ...
    def create_goal(self) -> None:
        ...
        if '/create' == self.msg.text and categories.count() > 0:
            ...
            category = GoalCategory(title=text, user=self.tg_user.user, board_id=board.id)
            category.save()
            self.tg_client.send_message(
                chat_id=self.msg.chat.id,
            ...
            category = GoalCategory.objects.filter(title__contains='tg', user_id=self.tg_user.user.id).first()
            goal = Goal(title=text, user=self.tg_user.user, category=category)
            goal.save()
